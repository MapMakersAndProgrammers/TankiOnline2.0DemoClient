package alternativa.tanks.game.utils
{
   ...
   {
      public function PhysicsParsingUtils()
      {
         super();
      }
      
      public static function parseBox(mesh:Mesh, collisionPrimitives:Vector.<CollisionPrimitive>, collisionGroup:int, collisionMask:int) : void
      {
      ...
      public static function parsePlane(mesh:Mesh, collisionPrimitives:Vector.<CollisionPrimitive>, collisionGroup:int, collisionMask:int) : void
      {
         var i:int = 0;
         ...
         var collisionRect:CollisionRect = new CollisionRect(new Vector3(width / 2,length / 2,0),collisionGroup,collisionMask);
      ...
      public static function parseCollisionTriangles(mesh:Mesh, collisionPrimitives:Vector.<CollisionPrimitive>, collisionGroup:int, collisionMask:int) : int
      {
         ...
         for(var i:int = 0; i < indices.length; i += 3)
         {
            ...
            numTriangles++;
            collisionPrimitives.push(createTriangle(vertices[0],vertices[1],vertices[2],mesh,collisionGroup,collisionMask));
         }
         return numTriangles;
      }
      
      private static function createTriangle(v0:Vector3, v1:Vector3, v2:Vector3, parentMesh:Mesh, collisionGroup:int, collisionMask:int) : CollisionTriangle
      {
         ...
         var y:Number = Number(Math.sqrt(yAxis.lengthSqr() - x * x));
         v2.reset(x,y,0);
         ...
         v2.x -= x;
         v2.y -= y;
         var collisionTriangle:CollisionTriangle = new CollisionTriangle(v0,v1,v2,collisionGroup,collisionMask);
