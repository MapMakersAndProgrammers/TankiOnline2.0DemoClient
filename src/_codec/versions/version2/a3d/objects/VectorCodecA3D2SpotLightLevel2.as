package _codec.versions.version2.a3d.objects
{
   ...
   public class VectorCodecA3D2SpotLightLevel2 implements ICodec
   {
      private var elementCodec:ICodec;
      
      private var optionalElement:Boolean;
      
      public function VectorCodecA3D2SpotLightLevel2(optionalElement:Boolean)
      {
         super();
      ...
      {
         var length2:int = 0;
         var items2:Vector.<A3D2SpotLight> = null;
         var i2:int = 0;
         var length1:int = LengthCodecHelper.decodeLength(protocolBuffer);
         var result:Vector.<Vector.<A3D2SpotLight>> = new Vector.<Vector.<A3D2SpotLight>>(length1,true);
         for(var i1:int = 0; i1 < length1; )
         {
            if(!protocolBuffer.optionalMap.get())
            {
               length2 = LengthCodecHelper.decodeLength(protocolBuffer);
               items2 = new Vector.<A3D2SpotLight>(length2,true);
               result[i1] = items2;
               for(i2 = 0; i2 < length2; i2++)
               {
                  items2[i2] = A3D2SpotLight(this.elementCodec.decode(protocolBuffer));
               }
            }
            i1++;
         }
      ...
      public function encode(protocolBuffer:ProtocolBuffer, object:Object) : void
      {
         ...
         var data:Vector.<Vector.<A3D2SpotLight>> = Vector.<Vector.<A3D2SpotLight>>(object);
