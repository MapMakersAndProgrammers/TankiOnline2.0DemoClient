package package_49
{
   ...
   public class VectorCodecA3D2SpriteLevel3 implements name_152
   {
      private var elementCodec:name_152;
      
      private var optionalElement:Boolean;
      
      public function VectorCodecA3D2SpriteLevel3(optionalElement:Boolean)
      {
         super();
      ...
      {
         var length2:int = 0;
         var items2:Vector.<Vector.<A3D2Sprite>> = null;
         var i2:int = 0;
         var length3:int = 0;
         var items3:Vector.<A3D2Sprite> = null;
         var i3:int = 0;
         var length1:int = name_443.name_445(protocolBuffer);
         var result:Vector.<Vector.<Vector.<A3D2Sprite>>> = new Vector.<Vector.<Vector.<A3D2Sprite>>>(length1,true);
         for(var i1:int = 0; i1 < length1; )
         {
            if(!protocolBuffer.optionalMap.name_447())
            {
               length2 = name_443.name_445(protocolBuffer);
               items2 = new Vector.<Vector.<A3D2Sprite>>(length2,true);
               result[i1] = items2;
               for(i2 = 0; i2 < length2; )
               {
                  if(!protocolBuffer.optionalMap.name_447())
                  {
                     ...
                     items3 = new Vector.<A3D2Sprite>(length3,true);
                     items2[i2] = items3;
                     for(i3 = 0; i3 < length3; i3++)
                     {
                        items3[i3] = A3D2Sprite(this.elementCodec.method_296(protocolBuffer));
                     }
                  }
                  i2++;
               }
      ...
      public function method_295(protocolBuffer:name_442, object:Object) : void
      {
         ...
         var data:Vector.<Vector.<Vector.<A3D2Sprite>>> = Vector.<Vector.<Vector.<A3D2Sprite>>>(object);
