package package_49
{
   ...
   public class VectorCodecA3D2JointBindTransformLevel2 implements name_152
   {
      private var elementCodec:name_152;
      
      private var optionalElement:Boolean;
      
      public function VectorCodecA3D2JointBindTransformLevel2(optionalElement:Boolean)
      {
         super();
      ...
      {
         var length2:int = 0;
         var items2:Vector.<A3D2JointBindTransform> = null;
         var i2:int = 0;
         var length1:int = name_443.name_445(protocolBuffer);
         var result:Vector.<Vector.<A3D2JointBindTransform>> = new Vector.<Vector.<A3D2JointBindTransform>>(length1,true);
         for(var i1:int = 0; i1 < length1; )
         {
            if(!protocolBuffer.optionalMap.name_447())
            {
               length2 = name_443.name_445(protocolBuffer);
               items2 = new Vector.<A3D2JointBindTransform>(length2,true);
               result[i1] = items2;
               for(i2 = 0; i2 < length2; i2++)
               {
                  items2[i2] = A3D2JointBindTransform(this.elementCodec.method_296(protocolBuffer));
               }
            }
            i1++;
         }
      ...
      public function method_295(protocolBuffer:name_442, object:Object) : void
      {
         ...
         var data:Vector.<Vector.<A3D2JointBindTransform>> = Vector.<Vector.<A3D2JointBindTransform>>(object);
